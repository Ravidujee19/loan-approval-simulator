...
#     - Derives annual/monthly income if only one is provided
#     - Filters out any fields not present on the SQLAlchemy model to avoid unexpected kwargs
#     - Returns 409 on constraint violations; 400 on invalid fields
#     """
#     data = payload.model_dump(exclude_none=True)
...
#     # Only pass columns that actually exist on the model
#     model_cols = {c.name for c in Applicant.__table__.columns}
#     filtered = {k: v for k, v in data.items() if k in model_cols}

#     # Always set owner
#     filtered["user_id"] = user["sub"]
...
#         db.commit()
#         db.refresh(applicant)
#         return ApplicantOut.model_validate(applicant)  # relies on model_config(from_attributes=True)
#     except IntegrityError:
#         db.rollback()
#         raise HTTPException(
...
#         )
#     except TypeError as e:
#         # Unexpected kwargs -> schema/DB mismatch
#         db.rollback()
#         log.exception("Applicant create TypeError: %s", e)
#         raise HTTPException(status_code=400, detail=f"Invalid field(s) for Applicant: {str(e)}")
#     except Exception as e:
#         db.rollback()
#         log.exception("Applicant create failed: %s", e)
#         raise HTTPException(status_code=500, detail="Internal error while creating applicant")
...
#         )
#     rows = db.execute(stmt).scalars().all()
#     return [ApplicantOut.model_validate(a) for a in rows]
...
#     return ApplicantOut.model_validate(app_obj)
...
#     # Keep only model columns to avoid attribute errors
#     model_cols = {c.name for c in Applicant.__table__.columns}
#     for k, v in list(data.items()):
#         if k not in model_cols:
#             data.pop(k)

#     # Derive missing income if needed (same logic as create)
#     if "monthly_income" in data and "annual_income" not in data:
#         data["annual_income"] = round(float(data["monthly_income"]) * 12, 2)
#     if "annual_income" in data and "monthly_income" not in data:
#         data["monthly_income"] = round(float(data["annual_income"]) / 12.0, 2)
